...
public:
	...
	void launch(fcmplx E1 = 1, fcmplx E2 = 0, float l = 1, float m = 0, float n = 0, float u = 0, float v = 0, float w = 1, float x = 0, float y = 0, float z = 0, float t = 0)
	{
		float p, q, r;
		xold = x;
		yold = y;
	...
	{ /* Scatter photon and establish new direction */
		...
		fcmplx s1, s2, e;
		float A[3][3];
		float mu, nu, phi, cosphi, sinphi, s1sq, s2sq, e1sq, e2sq, e12, F, a, b, c, Fmax;
		ncall++;
	...
	for (int i=0; i<3; i++)
	    for (int j=i; j<3; j++) {
		float P2 = (P[i][0]*P[j][0] + P[i][1]*P[j][1] + P[i][2]*P[j][2]);
	...
	void pointestimator(float *td, float *deposit, float Q[3][3], fcmplx *Ed1, fcmplx *Ed2, float zd, float ud = 0, float vd = 0, float wd = 1) const
	{
		float mu, nu, sinphi, cosphi, p[3], A[3][3], s1sq, s2sq, e1sq, e2sq, e12, F;
		fcmplx s1, s2;
		...
		if (fabs(1 - mu) < 1e-11)
		{ /* photon is already in the right direction */
			for (int i = 0; i < 3; i++)
				for (int j = 0; j < 3; j++)
					Q[i][j] = Pold[i][j];
		...
		else if (fabs(1 + mu) < 1e-11)
		{ /* photon is in the opposite direction */
...
public:
    ...
    */
	...
	float xold, yold, zold, told, x, y, z, t, weight, P[3][3], Pold[3][3];
	fcmplx E1, E2, E1old, E2old;
	fcmplx S1, S2; /* S1, S2 the Hulst scattering amplitude element used in scattering */
	int nsct;	   /* number of scattering events */

private:
	const scatterer *sct;
	float albedo, csca, psca0; /* psca0 = (|S2|^2 + |S1|^2)/2 with theta=0 */
	float bit;				   /* store the extra bits from roulette */
	float rejmu, rejphi;
	int ncall, N;
	mt_prng *mtrand;
};
