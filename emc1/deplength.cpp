...
const float lambda = 0.633; // micro
const float n_water = 1.33;
const float dia = 0.94;			// diameter in micro
const float x = 4;				//pi*dia/lambda*n_water;
const float mr = 1.2, mi = 0.0; // m=mr-i*mi is the refractive index

int main()
{
	fcmplx Ex, Ey, Ez, Ed1, Ed2;
	float td, deposit, Q[3][3];
	fcmplx m(mr, mi);
	scatterer sct(x, m);

	float L, Ipar, Iper, IxyR, IxyI;
