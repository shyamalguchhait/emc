...
const float lambda = 0.6328; // micro
const float dia = 2.0;       // micro
const float n_water = 1.33;
const float n_sct = 1.59;
const float xx = pi * dia / lambda * n_water; // size parameter of the Mie sphere
const float mr = n_sct / n_water, mi = 0.0;   // m=mr-i*mi is the refractive index
const float L = 4;                            // slab thickness in lsa
const float HW = 10;                          // the half width of the window in lsa
...
static inline void EtoS(fcmplx E1, fcmplx E2, float S[4])
{
    fcmplx E12;
    S[0] = 1;
    S[1] = norm(E1) - norm(E2);
...
static inline void mullerDeposit(float So[4], float Si[4], float phi, float weight, float M[4][4])
{
    float sin4phi = sin(4 * phi), cos4phi = cos(4 * phi);
    float D00 = 1, D11 = 3 - cos4phi, D12 = sin4phi, D21 = sin4phi, D22 = 3 + cos4phi, D33 = 4;
    for (int i = 0; i < 4; i++)
    {
...
int main()
{
    fcmplx Eix, Eiy;           // incident E field
    float alpha, beta, Si[4];  // the incident Stokes vector
    fcmplx Ed1, Ed2, Eox, Eoy; // outgoing E field
    float td, deposit, Q[3][3], So[4];
    fcmplx Ei1, Ei2, Eo1, Eo2; // incident and outgoing E with respect to the phi=0 plane
    float dt, x, y, rho, phi;
    int irho, nsct;
    float T = 0, R = 0, Lost = 0; // total transmission and reflection
    ...
    for (int i = 1; i <= photons; i++)
    {
        ...
        Eix = fcmplx(cos(alpha) * cos(beta), -cos(alpha) * sin(beta));
        Eiy = fcmplx(sin(alpha) * cos(beta), sin(alpha) * sin(beta));
        ph->launch(Eix, Eiy);
