...
const float lambda = 0.515;                   // micro
const float dia = 0.46;                       // micro
...
{
    fcmplx Ei1, Ei2; // incident E field
    float Si[4];
    fcmplx Ex, Ey; // outgoing E field
    float So[4];
    fcmplx Ed1, Ed2, phase; // used by pointEstimator
    float td, deposit, Q[3][3];
    int ix, iy;
    float T, R, Lost; // total transmission, reflection and lost photons
    ...
    for (int cno = 0; cno < 4; cno++)
    {
        ...
        {
            Ei1 = 1 / sqrt(2);
            Ei2 = 1 / sqrt(2); // 45 degree polarized
            if (argc == 1)
        ...
        {
            Ei1 = 1 / sqrt(2);
            Ei2 = fcmplx(0, 1 / sqrt(2)); // right circular
            if (argc == 1)
        ...
        for (int i = 1; i <= photons; i++)
        {
            ...
            while (1)
            {
                ...
                phase = fcmplx(cos(KK * td), sin(KK * td));
