...
public:
	...
	*s2 = s2tab[p];
	...
	if ( p == 1 ) return invtab[nslot];
	    ...
	    assert (P >= 0 && P < nslot);
	    return invtab[P] + (p-P/(0.+nslot))*(invtab[P+1] - invtab[P]);
	}
    }
...
private:
	...
	fprintf(stderr, "The last entry of the forward table (should be 1): %f \n", tab[N-1]);
	...
	for (int i=1; i<nslot; i++) {
	    p = i/(0. + nslot);
	    if (tab[j] > p) {
		fprintf(stderr, "Warning! Please increase the number of sampling angles.\n");
		exit(1);
	    }
	    else
		while (1) {
