...
public:
	...
	void pointestimator(double *td, double *deposit, double Q[3][3], dcmplx *Ed1, dcmplx *Ed2, double zd, double ud = 0, double vd = 0, double wd = 1) const
	{
		...
		/* the current propagating direction is (u,v,w) and detected in (ud, vd, wd) */
		mu = P[2][0] * ud + P[2][1] * vd + P[2][2] * wd;
		nu = sqrt(1 - mu * mu);
		...
		else
		{
			...
			p[0] = (P[2][1] * wd - P[2][2] * vd) / nu;
			p[1] = (P[2][2] * ud - P[2][0] * wd) / nu;
			p[2] = (P[2][0] * vd - P[2][1] * ud) / nu;

			cosphi = P[1][0] * p[0] + P[1][1] * p[1] + P[1][2] * p[2];
			sinphi = -(P[0][0] * p[0] + P[0][1] * p[1] + P[0][2] * p[2]);
			...
			*Ed1 = (cosphi * E1 + sinphi * E2) * s2 / sqrt(F);
			*Ed2 = (-sinphi * E1 + cosphi * E2) * s1 / sqrt(F);
		}
		...
		if (nsct == 0)
			if (fabs(1 - mu) < 1e-11)
				...
				*deposit = 0;
		else /* the collecting angle is pi/100 radian. The introdution of this collection angle DOES ONLY make a difference in the exact forward scattering direction. */
			*deposit = weight * F / csca * (2 * pi * (1 - cos(pi / 100))) * exp(-fabs((z - zd) / wd));
		*td = t + fabs((z - zd) / wd);
...
public:
    ...
    */
	double xold, yold, zold, told, x, y, z, t, weight, P[3][3];
	dcmplx E1, E2;
